public class TP01Q03_Ciframento {
    ...
    public static String cifrar(String charOriginal, int chave) { // metodo para mudar os caracteres
        char [] c = charOriginal.toCharArray(); 
        for (int i = 0; i < charOriginal.length(); i++) {
            c[i] += 3;
        }
     
        return String.valueOf(c); // retorna a string
    }
    ...
    public static void main(String[] args){S
        ...
        for(int i = 0 ; i < numEntrada; i++){
            MyIO.println(cifrar(entrada[i], chave)); // imprime a palavra cifrada
        }
